...
import ownkey.notification.presentation.response.SettingsResponse;
...
public class SettingsController {
    ...
    public ResponseEntity<SettingsResponse> getNotificationSettings(
            @AuthenticationPrincipal UserPrincipal userPrincipal
    ) {
        SettingsResult result = notificationUseCase.getSettings(userPrincipal.userId());

        return ResponseEntity.ok(new SettingsResponse(
                result.community(), result.wiki(), result.showmethekey()
        ));
    }
    ...
    public ResponseEntity<Void> updateNotificationSettings(
        ...
        UpdateSettingsCommand command = new UpdateSettingsCommand(
                request.community(), request.wiki(), request.showmethekey()
        );
    ...
    public ResponseEntity<Void> registerDevice(
        ...
        RegisterDeviceCommand command = new RegisterDeviceCommand(
                request.deviceToken(), request.environment()
        );

        deviceTokenUseCase.registerDevice(userPrincipal.userId(), command);
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Void> unregisterDevice(
        ...
        deviceTokenUseCase.unregisterDevice(userPrincipal.userId(), deviceId);
        return ResponseEntity.noContent().build();
    }
}